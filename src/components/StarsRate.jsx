      ...
      <div style={{display: 'flex', gap: '4px'}}>
        {Array.from({length: maxRate}, (_, i) =>(
          <Star 
            ...
            onLeaveStar={()=>{setTempRaiting(0)}}
            onEnterStar={()=>{setTempRaiting(i+1)}}
            color={color}
            size={size}
          />
        ))}
      </div>
      <p style={{fontSize: `${size / 1.5 }px`}}>{tempRaiting || rating || ""}</p>
    </div>
  )
}

function Star({onRate, full, onLeaveStar, onEnterStar, color, size}) {  
  return (
    <span 
